...
特点：
  ...
  ✅ OCR引擎支持 EasyOCR / GLM-OCR 二选一（配置项切换）
"""
import os
import json
import numpy as np
...
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # 强制CPU避免驱动问题，GLM-OCR使用时可改为 "0"
# ======== 以上必须在import torch之前 ========
...
# 🔧 OCR引擎配置（二选一）
OCR_ENGINE = "easyocr"  # 可选: "easyocr" 或 "glm-ocr"
GLM_OCR_MODEL_PATH = "./models/GLM-OCR"  # GLM-OCR本地路径，或 "ZhipuAI/GLM-OCR" 在线加载
# ======== 配置结束 ========
...
class TextExtractor:
    def __init__(self):
        ...
        self.ocr_engine = None  # EasyOCR 懒加载
        self.glm_ocr_model = None  # GLM-OCR 模型懒加载
        self.glm_ocr_processor = None  # GLM-OCR 处理器懒加载
    ...
    def _init_ocr(self):
        """懒加载OCR引擎（支持 EasyOCR / GLM-OCR 二选一）"""
        if OCR_ENGINE == "easyocr":
            ...
            try:
                ...
                print(f"  → OCR设备选择: {'GPU' if use_gpu else 'CPU'} (EasyOCR)")
                ...
                raise RuntimeError(f"EasyOCR初始化失败: {str(e)}")
        
        elif OCR_ENGINE == "glm-ocr":
            if self.glm_ocr_model is not None and self.glm_ocr_processor is not None:
                return self.glm_ocr_model, self.glm_ocr_processor
            
            print("  → 首次使用OCR，加载GLM-OCR模型（约30-60秒）...")
            try:
                from modelscope import AutoProcessor, AutoModelForImageTextToText
                use_gpu = torch.cuda.is_available()
                print(f"  → OCR设备选择: {'GPU' if use_gpu else 'CPU'} (GLM-OCR)")
                
                self.glm_ocr_processor = AutoProcessor.from_pretrained(
                    GLM_OCR_MODEL_PATH, trust_remote_code=True
                )
                self.glm_ocr_model = AutoModelForImageTextToText.from_pretrained(
                    pretrained_model_name_or_path=GLM_OCR_MODEL_PATH,
                    torch_dtype="auto" if use_gpu else torch.float32,
                ...
                raise ImportError("❌ 未安装modelscope，请先安装: pip install modelscope transformers accelerate")
            except Exception as e:
                raise RuntimeError(f"GLM-OCR初始化失败: {str(e)}")
        
        else:
            raise ValueError(f"不支持的OCR引擎: {OCR_ENGINE}，请选择 'easyocr' 或 'glm-ocr'")
    
    def _ocr_with_easyocr(self, image) -> str:
        """使用 EasyOCR 识别单张图片"""
        ocr = self._init_ocr()  # 返回 EasyOCR Reader
    ...
    def _ocr_with_glm_ocr(self, image_path: str) -> str:
        """使用 GLM-OCR 识别单张图片（返回纯文本）"""
        model, processor = self._init_ocr()  # 返回 (model, processor)
        
        # 确保图片路径是绝对路径
        abs_image_path = os.path.abspath(image_path)
        
        # 构建 messages（GLM-OCR 官方格式）
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "image", "url": abs_image_path},
                    {"type": "text", "text": "Text Recognition:"}  # 文档解析 prompt
                ],
            }
        ]
        ...
        try:
            ...
            print(f"    ⚠️ GLM-OCR 单页识别失败: {str(e)[:80]}")
            return ""
    
    def _parse_pdf_with_ocr(self, filepath: str) -> Optional[List[Dict]]:
        """OCR方案：处理扫描件PDF（支持 EasyOCR / GLM-OCR 二选一）"""
        if not USE_OCR:
        ...
        try:
            ...
            # PDF转图片
            print(f"  → OCR处理中（PDF转图片，引擎: {OCR_ENGINE}）...")
            ...
            for page_num, image in enumerate(images, 1):
                ...
                if OCR_ENGINE == "easyocr":
                    ...
                    # GLM-OCR 需要保存临时图片文件（因为它通过路径读取）
                    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                        tmp_path = tmp.name
                        image.save(tmp_path, format="PNG")
                    
                    try:
                        text = self._ocr_with_glm_ocr(tmp_path)
                    finally:
                        # 清理临时文件
            ...
            print(f"\n  ✓ OCR完成 {len(pages)} 页 (引擎: {OCR_ENGINE})")
            return pages if any(p["text"].strip() for p in pages) else None
    ...
    def _parse_pdf(self, filepath: str) -> List[Dict]:
        ...
        if USE_OCR:
            print(f"  → 回退到OCR（扫描件PDF，引擎: {OCR_ENGINE}）...")
